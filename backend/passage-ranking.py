...
def magic(args: Dict):
    ...
    for document in collection_doc.find({}):
        ...
        if "Embeddings" not in document or len(document["Embeddings"]) != len(document["Blocks"]):
            embeddings = [emb.tolist() for emb in model.encode(document["Blocks"])]
            print("COUNT "+ str(len(embeddings)))
            collection_doc.update_one({"_id": document["_id"]}, {"$set": {"Embeddings": embeddings}})

            # Convert embeddings to NumPy arrays
            embeddings = [np.array(block_emb, dtype=np.float32) for block_emb in embeddings]
        
        # Convert existing embeddings to NumPy arrays
        else:
            embeddings = [np.array(block_emb, dtype=np.float32) for block_emb in document["Embeddings"]]
    ...
    blocks_score_pairs = sorted(blocks_score_pairs, key=lambda x: x[1], reverse=True)
